"""ExQ Compiled Runtime: executes inference using compiled artifacts.

This is what makes ExQ a compiler, not a planner. The runtime:
1. Loads a model + ExQ compiled artifact
...
Usage (transformers integration — drop-in patch):
    from exq.runtime import exq_patch, load_exq_model
    model = exq_patch(model, "artifacts/model.json")  # patch existing model
    model, tok = load_exq_model("Qwen/Qwen2.5-3B", "artifacts/dense/qwen2.5-3b-512.json")
"""
...
from exq.runtime.coverage_monitor import CoverageSnapshot, OnlineMonitor


class ExpertWeightCache:
    """Two-tier expert weight cache: GPU (hot) + CPU (cold).

    Hot experts (BF16/INT8 per ExQ plan) stay GPU-resident.
...
class CompiledMoEForward(nn.Module):
    """Drop-in replacement for MoE forward pass using ExQ compiled schedule.
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs) -> Any:
        ...
        # Issue prefetches BEFORE compute (overlaps with router + expert execution)
         # This is the compiled schedule — no prediction model running
    ...
    def _issue_prefetches(self, hidden_states: torch.Tensor):
        ...
        # Fast-path: if nothing is offloaded, there is nothing to prefetch.
         # This keeps the compiled runtime at low overhead on
...
class CompiledInference:
    ...
    def from_artifact(
        ...
        """Create a compiled inference engine from an ExQ artifact."""
        with open(artifact_path, encoding="utf-8") as f:
            artifact = json.load(f)

        engine = cls(model, tokenizer, artifact, device)
        patched = engine.patch_model()
        print(f"ExQ compiled runtime: {patched} MoE layers patched")
...
def benchmark_compiled_vs_baseline(
    ...
    """Head-to-head: baseline model vs ExQ compiled model.
...
# ── Transformers integration (drop-in exq_patch) ────────────────────────

from exq.runtime.transformers_integration import load_exq_model, exq_patch

__all__ = [
    "CompiledInference",
    "CompiledMoEForward",
    "ExpertWeightCache",
    "benchmark_compiled_vs_baseline",
    "load_exq_model",
    "exq_patch",
]
