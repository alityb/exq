...
from exq.eval.quant_shim import quantize_tensor_int4, quantize_tensor_int8
from exq.model_utils import find_attention_module, find_transformer_layers
from exq.profiler.attention_profiler import _find_attention_layers
...
def apply_dense_quant(model: nn.Module, plan: DenseQuantPlan) -> nn.Module:
    ...
    counts: Counter[str] = Counter()
    layers_container = find_transformer_layers(model)
    ...
    for layer_idx, layer in enumerate(layers_container):
        ...
        attn = find_attention_module(layer)
        if attn is None:
            continue
