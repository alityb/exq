...
def bench(
    ...
    """GPU kernel benchmark: warm up, then time n_runs calls.

    Returns dict with p50, p75, p95, p99, mean, std, ci95 (all in ms).
    """
    ...
    return {
        ...
        "p99":  times[min(int(0.99 * (n - 1)), n - 1)],
...
def compute_quant_diff(profile_path: str | Path) -> float:
    """Compile-time diagnostic: fraction of experts assigned INT8 or BF16.

    Profiles with high quant_diff have concentrated routing and will benefit
    from ExQ's mixed-precision assignment. Near-zero means near-uniform routing.
    """
    from exq._core import RoutingProfile, py_build_routing_graph
    from exq import CompilerPipeline
    ...
    if not plan:
        return 0.0
    non_int4 = sum(1 for v in plan.values() if v in ("INT8", "BF16"))
    return non_int4 / len(plan)
...
def compute_recovery_pct(fp16_ppl: float, exq_ppl: float, int4_ppl: float) -> float:
    """Recovery percentage (0–100). Returns 0.0 when INT4 is not worse than fp16."""
    degradation = int4_ppl - fp16_ppl
