"""GPTQ implementation for ExQ: Hessian-aware weight quantization.
...
ExQ's routing-informed mixed precision.
...
def gptq_quantize_experts_mixed(
    ...
    """Apply GPTQ with ExQ's routing-informed precision assignments.

    Hot experts: 8-bit GPTQ (more precision where it matters)
    Cold experts: 4-bit GPTQ (aggressive compression where it doesn't)

    This is ExQ + GPTQ: the compiler decides WHERE to allocate bits,
