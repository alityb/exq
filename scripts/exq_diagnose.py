#!/usr/bin/env python3
"""ExQ Diagnostic: analyze a model and predict optimization benefit in <3 seconds.

Usage:
    python scripts/exq_diagnose.py --profile profiles/olmoe.json
    python scripts/exq_diagnose.py --profile profiles/qwen3-30b-a3b.json

This is the compile-time diagnostic that tells you WHETHER to deploy ExQ
...
def main():
    parser = argparse.ArgumentParser(
        description="ExQ: Compile-time deployment diagnostic"
    )
    ...
    parser.add_argument("--output-dir", default="results/compiled/", help="Kernel output directory")
    ...
    print()
    print("=" * 66)
    print(f"  ExQ Deployment Diagnostic")
    ...
    if quant_benefit == "HIGH":
        ...
        print(f"    → RECOMMENDATION: Deploy ExQ mixed precision")
    ...
    if args.emit_kernels:
        ...
        print(f"  Manifest: {Path(args.output_dir) / 'exq_manifest.json'}")
