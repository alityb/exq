"""Fig 3: KL divergence vs fp16, Qwen2.5-3B, WikiText2."""
...
variants = ["Uniform INT4", "ExQ (ours)", "AWQ"]
mean_kl  = [kl_i4["mean"], kl_exq["mean"], kl_awq["mean"]]
p99_kl   = [kl_i4["p99"],  kl_exq["p99"],  kl_awq["p99"]]
colors   = [CORAL, TEAL, BLUE]

x, width = np.arange(len(variants)), 0.35
fig, ax = plt.subplots(figsize=(7, 4.5))

bars_mean = [ax.bar(xi - width/2, mean_kl[i], width, color=colors[i], zorder=3)
             for i, xi in enumerate(x)]
bars_p99  = [ax.bar(xi + width/2, p99_kl[i],  width, color=colors[i],
                    alpha=0.45, hatch="///", zorder=3, edgecolor=colors[i])
             for i, xi in enumerate(x)]
...
ax.set_ylabel("KL divergence vs fp16")
ax.set_xticks(x)
ax.set_xticklabels(variants)
ax.set_title("Output fidelity  (Qwen2.5-3B, WikiText2)", pad=10)
ax.legend(handles=[
...
ax.annotate(
    f"{reduction:.0f}x lower than AWQ",
